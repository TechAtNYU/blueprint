...
.controller('Fall2015Ctrl', function($scope, Restangular, moment) {
    ...
    function reduceArraytoValue (values) {
        ...
        .then(function(data) {
            ...
            _(events).forEach(function (val) {
                ...
                if (val.relationships && val.relationships.attendees && val.relationships.attendees.data && val.relationships.attendees.data.length > 1) {
                    ...
                    var dateComparison = currentEventTime.isAfter(fourMonthsAgo);
                    if (dateComparison) {
            ...
            $scope.totalEvents = reduceArraytoValue(eventsWithCheckinsDataset[0].data);
            ...
            $scope.totalCheckins = reduceArraytoValue(checkinDataset[0].data);
            $scope.totalRSVPs = reduceArraytoValue(checkinDataset[1].data);
    ...
    Restangular.one('people?sort=+created')
        ...
        .then(function(people) {
            ...
            _(people).forEach(function (val) {
                ...
                var dateComparison = currentPersonJoinDate.isAfter(fourMonthsAgo);
                    if (dateComparison) {
            ...
            $scope.newPeople = reduceArraytoValue(newPeopleDataset[0].data);
